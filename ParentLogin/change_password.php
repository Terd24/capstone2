...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($new_password) || empty($confirm_password)) {
        ...
        $stmt = $conn->prepare("UPDATE parent_account SET password = ?, must_change_password = 0 WHERE parent_id = ?");
        $stmt->bind_param("si", $hashed_password, $_SESSION['parent_id']);
        
        if ($stmt->execute()) {
            $success = 'Password changed successfully! Redirecting...';
            header("refresh:2;url=ParentDashboard.php");
        } else {
