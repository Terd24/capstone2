...
if (!empty($cashier_id)) {
    $stmt = $conn->prepare("SELECT * FROM cashier_account WHERE id_number = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_cashier'])) {
    $delete_stmt = $conn->prepare("DELETE FROM cashier_account WHERE id_number = ?");
    $delete_stmt->bind_param("s", $cashier_id);
    
    if ($delete_stmt->execute()) {
        $_SESSION['success_msg'] = "Cashier account deleted successfully!";
        header("Location: ../AccountList.php?type=cashier");
        exit;
    } else {
        $error_msg = "Error deleting cashier account.";
    }
    $delete_stmt->close();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_cashier'])) {
    ...
    if (!empty($password)) {
        ...
        $update_stmt->bind_param(
            ...
            $first_name, $last_name, $middle_name,
            $dob, $birthplace, $gender, $address,
            $id_number, $username, $hashed_password, $cashier_id
        );
    } else {
...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden border-2 border-[#0B2C62] transform transition-all scale-100" id="modalContent">
        ...
        <div class="flex justify-between items-center border-b border-gray-200 px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50">
            <h2 class="text-lg font-semibold text-gray-800">Cashier Information</h2>
        ...
        <form id="cashierForm" method="POST" class="px-6 py-6 grid grid-cols-1 md:grid-cols-3 gap-6 overflow-y-auto max-h-[80vh] no-scrollbar">
            ...
            <div>
                ...
                <input type="text" name="password" placeholder="Enter new password" readonly class="w-full border border-gray-300 px-3 py-2 rounded-lg bg-gray-50 cashier-field">
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
            <div class="p-6">
                ...
                <h3 class="text-lg font-semibold text-gray-900 text-center mb-2">Delete Cashier Account</h3>
                <p class="text-gray-600 text-center mb-6">Are you sure you want to delete this cashier account? This action cannot be undone.</p>
...
function proceedDelete() {
    ...
    form.method = 'POST';
    form.innerHTML = '<input type="hidden" name="delete_cashier" value="1">';
